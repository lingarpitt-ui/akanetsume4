    ...
    const handleExtractFromResume = async () => {
        ...
        reader.onloadend = async () => {
            ...
            try {
                ...
                setModalMessage(`Successfully extracted ${newHistory.length} job position(s) from your resume.`);
            } catch (error) {
...
// ... rest of the function remains the same
    ...
    const handleJobSubmit = async (e) => {
        e.preventDefault();
        const { id, ...jobData } = currentJob; 
        
        if (!jobData.company || !jobData.jobTitle || !jobData.startDate) {
            setModalMessage("Please fill in Company, Job Title, and Start Date.");
            return;
        }

        const historyCollection = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('employmentHistory');
        if (isEditingJob) {
            await historyCollection.doc(id).update(jobData);
        } else {
            jobData.order = employmentHistory.length;
            await historyCollection.add(jobData);
        }
        setCurrentJob({ id: null, company: '', jobTitle: '', startDate: '', endDate: '', city: '', description: '' });
        setIsEditingJob(false);
    };
    
    const handleAccreditationChange = (e) => {
        const { name, value } = e.target;
        setCurrentAccreditation(prev => ({ ...prev, [name]: value }));
    };

    const handleAccreditationSubmit = async (e) => {
        e.preventDefault();
        const { id, ...accData } = currentAccreditation;

        if (!accData.name || !accData.institute || !accData.year) {
            setModalMessage("Please fill in all accreditation fields.");
            return;
        }
        const accCollection = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('accreditations');
        if (isEditingAccreditation) {
            await accCollection.doc(id).update(accData);
        } else {
            accData.order = accreditations.length;
            await accCollection.add(accData);
        }
        setCurrentAccreditation({ id: null, name: '', institute: '', location: '', year: '' });
        setIsEditingAccreditation(false);
    };

    const handleDragEnd = async (list, collectionName) => {
        const batch = db.batch();
        const collectionRef = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection(collectionName);
    ...
    const createDragHandlers = (list, setList, collectionName) => ({
        onDragStart: (e, position) => { dragItem.current = position; },
        onDragEnter: (e, position) => { dragOverItem.current = position; },
        onDragEnd: () => {
            ...
            dragOverItem.current = null;
            setList(newList);
            handleDragEnd(newList, collectionName);
        },
    });
